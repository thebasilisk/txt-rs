...
use utils::{get_library, get_next_frame, init_render_with_bufs, make_buf, new_render_pass_descriptor, prepare_pipeline_state, simple_app};
    ...
    loop {
        autoreleasepool(|_| {
            ...
            if unsafe { frame_time.compare(&NSDate::now()) } == NSComparisonResult::Ascending {
                ...
                encoder.set_vertex_bytes(0, (size_of::<Float4>()) as u64 * 2, vec![Float4(view_width as f32, view_height as f32, 0.0, 0.0), Float4(153.0, 0.0, 0.0, 0.0)].as_ptr() as *const _);
                encoder.set_vertex_bytes(1, (size_of::<vertex_t>() * vertex_data.len()) as u64, vertex_data.as_ptr() as *const _);
                encoder.set_vertex_bytes(2, (size_of::<Float2>()) as u64, vec![Float2(0.0, 0.0)].as_ptr() as *const _);
                encoder.set_fragment_texture(0, Some(&char_tex));
                encoder.draw_primitives(metal::MTLPrimitiveType::Triangle, 0, vertex_data.len() as u64);
...
fn build_rect (x : f32, y : f32, width : f32, height : f32, rot : f32) -> Vec<vertex_t> {
    ...
    let vert1 = vertex_t{position: Float4(v1_rot_pos.0, v1_rot_pos.1, 0.0, 1.0), uv: Float4(0.0, 1.0, 0.0, 0.0),color: Float4(0.3, 0.3, 0.3, 1.0)};
    ...
    let vert2 = vertex_t{position: Float4(v2_rot_pos.0, v2_rot_pos.1, 0.0, 1.0), uv : Float4(1.0, 1.0, 0.0, 0.0), color: Float4(0.3, 0.3, 0.3, 1.0)};
    ...
    let vert3 = vertex_t{position: Float4(v3_rot_pos.0, v3_rot_pos.1, 0.0, 1.0), uv : Float4(0.0, 0.0, 0.0, 0.0), color: Float4(0.3, 0.3, 0.3, 1.0)};
    ...
    let vert4 = vertex_t{position: Float4(v4_rot_pos.0, v4_rot_pos.1, 0.0, 1.0), uv : Float4(1.0, 0.0, 0.0, 0.0), color: Float4(0.3, 0.3, 0.3, 1.0)};
