use std::{ptr, slice};
use freetype::{freetype::{FT_Face, FT_Init_FreeType, FT_Library, FT_Load_Char, FT_New_Face, FT_Set_Char_Size, FT_Set_Pixel_Sizes, FT_LOAD_RENDER}, succeeded, FT_Error};
...
fn load_typeface(lib: FT_Library, name: &str) -> Result<FT_Face, FT_Error>{
    ...
    let result = unsafe {
        FT_New_Face(lib, filepath.as_ptr() as *const _, 0, &mut face)
    };
    if succeeded(result) {
        Ok(face)
    } else {
...
pub fn get_char_glyph(font: &str, character: char) -> Result<(*mut u8, u64, u64), FT_Error>{
    ...
    let result = unsafe {
        FT_Load_Char(face, character as u64, FT_LOAD_RENDER as i32)
    };
